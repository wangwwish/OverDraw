...
import com.wwish.ganalytics.okhttp.adapter.CacheCall;
...
import com.wwish.ganalytics.okhttp.convert.Converter;
...
public abstract class BaseRequest<R extends BaseRequest> {
    ...
    public String getCacheKey() {
        if(TextUtils.isEmpty(cacheKey)){
            //添加默认缓存key
            cacheKey = HttpUtils.createUrlFromParams(getBaseUrl(), getParams().urlParamsMap);
        }
        return cacheKey;
    }
    ...
    public okhttp3.Call generateCall(Request request) {
        ...
        if (readTimeOut <= 0 && writeTimeOut <= 0 && connectTimeout <= 0 && sslParams == null && userCookies.size() == 0) {
            ...
            if (userCookies.size() > 0) OkHttp.getInstance().getCookieJar().addCookies(userCookies);
