...
import com.wwish.ganalytics.okhttp.cache.CacheManager;
import com.wwish.ganalytics.okhttp.cache.CacheMode;
import com.wwish.ganalytics.okhttp.callback.AbsCallback;
import com.wwish.ganalytics.okhttp.callback.AbsCallbackWrapper;
import com.wwish.ganalytics.okhttp.exception.ExceptionConstans;
...
import com.wwish.ganalytics.okhttp.utils.HeaderParser;
...
public class CacheCall<T> implements Call<T> {
        ...
        if (cacheMode == CacheMode.IF_NONE_CACHE_REQUEST) {
            ...
            if (cacheEntity != null && !cacheEntity.isExpire()) {
                ...
                if (data == null || headers == null) {
                    //由于没有序列化等原因,可能导致数据为空
                    sendFailResultCallback(true, OkHttpException.INSTANCE(ExceptionConstans.CODE_A0001,ExceptionConstans.MSG_A0001));
                } else {
                    sendSuccessResultCallback(true, data);
                    return;//获取缓存成功,不请求网络
                }
            } else {
                sendFailResultCallback(true, OkHttpException.INSTANCE(ExceptionConstans.CODE_A0002,ExceptionConstans.MSG_A0002));
            }
            ...
            if (cacheEntity != null && !cacheEntity.isExpire()) {
                ...
                if (data == null || headers == null) {
                    //由于没有序列化等原因,可能导致数据为空
                    sendFailResultCallback(true, OkHttpException.INSTANCE(ExceptionConstans.CODE_A0003,ExceptionConstans.MSG_A0003));
                } else {
                    sendSuccessResultCallback(true, data);
                }
            } else {
                sendFailResultCallback(true, OkHttpException.INSTANCE(ExceptionConstans.CODE_A0004,ExceptionConstans.MSG_A0004));
            }
        }
        ...
        rawCall.enqueue(new okhttp3.Callback() {
            ...
            public void onFailure(okhttp3.Call call, IOException e) {
                if (e instanceof SocketTimeoutException && currentRetryCount < baseRequest.getRetryCount()) {
                    ...
                    mCallback.parseError(e);
                    //请求失败，一般为url地址错误，网络错误等,并且过滤用户主动取消的网络请求
                    if (!call.isCanceled()) {
                        sendFailResultCallback(false, OkHttpException.INSTANCE(ExceptionConstans.CODE_A0005,e.getLocalizedMessage()));
                    }
                }
            }
            ...
            public void onResponse(okhttp3.Call call, okhttp3.Response response) throws IOException {
                ...
                if (responseCode == 304 && cacheMode == CacheMode.DEFAULT) {
                    if (cacheEntity == null) {
                        sendFailResultCallback(true, OkHttpException.INSTANCE(ExceptionConstans.CODE_A0006,ExceptionConstans.MSG_A0006));
                    } else {
                        T data = cacheEntity.getData();
                        HttpHeaders headers = cacheEntity.getResponseHeaders();
                        if (data == null || headers == null) {
                            //由于没有序列化等原因,可能导致数据为空
                            sendFailResultCallback(true, OkHttpException.INSTANCE(ExceptionConstans.CODE_A0007,ExceptionConstans.MSG_A0007));
                        } else {
                            sendSuccessResultCallback(true, data);
                        }
                    }
                ...
                if (responseCode == 404 || responseCode >= 500) {
                    sendFailResultCallback(false, OkHttpException.INSTANCE(ExceptionConstans.CODE_A0008,ExceptionConstans.MSG_A0008));
                    return;
                }
                ...
                try {
                    ...
                    //网络请求成功回调
                    sendSuccessResultCallback(false, data);
                } catch (Exception e) {
                    //一般为服务器响应成功，但是数据解析错误
                    if(e instanceof OkHttpException){
                        sendFailResultCallback(false, OkHttpException.INSTANCE(((OkHttpException) e).errorCode,(((OkHttpException) e).errorMsg)));
                    }else{
                        sendFailResultCallback(false, OkHttpException.INSTANCE(ExceptionConstans.CODE_A0009,e.getLocalizedMessage()));
                    }
    ...
    /** 失败回调，发送到主线程 */
    @SuppressWarnings("unchecked")
    private void sendFailResultCallback(final boolean isFromCache,final OkHttpException e) {
        final CacheMode cacheMode = baseRequest.getCacheMode();
        ...
        OkHttp.getInstance().getDelivery().post(new Runnable() {
            ...
            public void run() {
                if (isFromCache) {
                    mCallback.onCacheError(e);             //缓存失败回调 （UI线程）
                    if (cacheMode == CacheMode.DEFAULT || cacheMode == CacheMode.REQUEST_FAILED_READ_CACHE) {
                        mCallback.onAfter();              //请求结束回调 （UI线程）
                    }
                } else {
                    mCallback.onError(e);        //请求失败回调 （UI线程）
        ...
        if (!isFromCache && cacheMode == CacheMode.REQUEST_FAILED_READ_CACHE) {
            if (cacheEntity != null && !cacheEntity.isExpire()) {
                ...
                if (data == null || headers == null) {
                    //由于没有序列化等原因,可能导致数据为空
                    sendFailResultCallback(true, OkHttpException.INSTANCE(ExceptionConstans.CODE_A0010,ExceptionConstans.MSG_A0010));
                } else {
                    sendSuccessResultCallback(true, data);
                }
            } else {
                sendFailResultCallback(true, OkHttpException.INSTANCE(ExceptionConstans.CODE_A0011,ExceptionConstans.MSG_A0011));
            }
        }
    }

    /** 成功回调，发送到主线程 */
    private void sendSuccessResultCallback(final boolean isFromCache, final T t) {
        final CacheMode cacheMode = baseRequest.getCacheMode();
